...
vector<int>* getPathDFS(int** graph, int v, int v1, int v2, bool* visited)
{
    if(v1 == v2)
    {
    ...
    for(int i=0; i<v; i++)
    {
        ...
        {
            vector<int>* temp = getPathDFS(graph, v, i, v2, visited);
            if(temp != NULL)
            {
                temp->push_back(v1);
                return temp;
...
int main()
{
    ...
    while(t--)
    {
        ...
        vector<int>* ans = getPathDFS(graph, v, v1, v2, visited);
        if(ans)
        {
