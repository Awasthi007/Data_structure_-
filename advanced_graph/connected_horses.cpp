...
ll dfs(int** chess, int i, int j, int n, int m)
{
    int count = 1;
    chess[i][j] = 0;
    
    
    if(i-1 >= 1 && j-2 >= 1 && chess[i-1][j-2] == 1)
    {
        count += dfs(chess, i-1, j-2, n ,m);
    }
    if(i-1 >= 1 && j+2 <= m && chess[i-1][j+2] == 1)
    {
        count += dfs(chess, i-1, j+2, n, m);
    }
    if(i-2 >= 1 && j-1 >= 1 && chess[i-2][j-1] == 1)
    {
        count += dfs(chess, i-2, j-1, n, m);
    }
    if(i-2 >= 1 && j+1 <= m && chess[i-2][j+1] == 1)
    {
        count += dfs(chess, i-2, j+1, n, m);
    }
    if(i+1 <= n && j-2 >= 1 && chess[i+1][j-2] == 1)
    {
        count += dfs(chess, i+1, j-2, n, m);
    }
    if( i+2 <= n && j-1 >= 1 && chess[i+2][j-1] == 1)
    {
        count += dfs(chess, i+2, j-1, n, m);
    }
    if(i+1 <= n && j+2 <= m && chess[i+1][j+2] == 1)
    {
        count += dfs(chess, i+1, j+2, n, m);
    }
    if(i+2 <= n && j+1 <= m && chess[i+2][j+1] == 1)
    {
        count += dfs(chess, i+2, j+1, n, m);
    }
...
ll solve(int** chess, int n, int m)
{ 
    ll ans = 1;
    for(int i=1; i<=n; i++)
    {
